...
if [ "$1" = "api" ]; then
  ...
  cdr_database_port=$(jq -r '.cdr_database.port' /opt/nethvoice-report/api/conf.json)
  ...
  cdr_database_user=$(jq -r '.cdr_database.user' /opt/nethvoice-report/api/conf.json)
  cdr_database_password=$(jq -r '.cdr_database.password' /opt/nethvoice-report/api/conf.json)
  cdr_database_name=$(jq -r '.cdr_database.name' /opt/nethvoice-report/api/conf.json)
